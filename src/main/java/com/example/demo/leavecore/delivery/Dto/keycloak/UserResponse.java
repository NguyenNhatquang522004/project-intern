...
public record UserResponse(
        ...
        Instant createdAt,
        Map<String, String> attributes // Đã được phẳng hóa (Flattened) để Frontend dễ dùng
) {

    /**
     * Static Factory Method - Chuyển đổi từ UserRepresentation sang UserResponse.
     * Giải quyết toàn bộ các vấn đề về Null-Safety và Convert kiểu dữ liệu cũ của
     * Keycloak.
     */
        ...
        if (userRep.getAttributes() != null) {
            flattenedAttributes = userRep.getAttributes().entrySet().stream()
                    ...
                    .collect(Collectors.toMap(
                            Map.Entry::getKey,
                            entry -> entry.getValue().get(0) // Lấy phần tử đầu tiên của List
                    ));
        }
